...
import { validateMultitool } from '../utils/multitool_validators.js'
...
router.put('/', async (req, res) => {
  validateMultitool(req.body, false, async (err, validBase) => {
    if (err) {
...
router.put('/edit', async (req, res) => {
  validateMultitool(req.body, true, async (err, validInfo) => {
    if (err) {
      res.status(err.status).json({ error: true, message: err.message })
      return
    }
